...
nhdRootDir = "E:\\NHDPlusv21"
global workspaceDir
workspaceDir = "L:/Priv/CORFiles/Geospatial_Library/Data\Project/wr61682_weber_SplitCatchment/Script1_FullSet_NewMethod"
...
for row in rows:
    ...
    if not os.path.exists(newDirectory):
        ...
        if not arcpy.Exists(newDirectory + "\\catchment.shp"):
        ...
        if arcpy.Exists(newDirectory + "\\point"):
            arcpy.Delete_management(newDirectory + "\\point")
    ...
    ##            outSnap = SnapPourPoint(newDirectory + "\\point",newDirectory + "\\point",100)
    ##            outSnap.save(newDirectory + "\\snappnt")
        arcpy.ClearEnvironment("extent")

        # Create watershed and calculate area
        if arcpy.Exists(newDirectory + "\\wshed"):
            arcpy.Delete_management(newDirectory + "\\wshed")
        outWatershed = Watershed(newDirectory + "\\fdr",newDirectory + "\\point")
